...
public:
    ...
    double torque_cmd_body[3]; //!< trick_units(N*m)
    /** Translational force demand, ECI frame (matches Control::out.force). */
    double force_cmd_eci[3]; //!< trick_units(N)
    /** Local geomagnetic field, body frame (T). */
    double B_body[3]; //!< trick_units(T)

    /**
     * Added to RCS body force after ECI→body rotation of force_cmd_eci.
     * Use for open-loop tests or disturbances (default zero).
     */
    double rcs_force_body_injection[3]; //!< trick_units(N)

    /**
     * Portion of torque_cmd_body sent to each path (each in [0,1]).
     * Defaults: wheels = 1, MTQ = 0, RCS torque = 0 (RCS does translation only).
     */
    double rw_torque_fraction; //!< trick_units(--)
    double mtq_torque_fraction; //!< trick_units(--)
    double rcs_torque_fraction; //!< trick_units(--)

    /** Bitwise OR of GncEffectorMask; disabled actuators are zeroed. */
    unsigned effector_enable_mask; //!< trick_units(--)

    GncActuatorCmdSource rcs_cmd_source; //!< trick_units(--)
    GncActuatorCmdSource rw_cmd_source; //!< trick_units(--)
    GncActuatorCmdSource mtq_cmd_source; //!< trick_units(--)

    // Outputs — connect to actuators (RCS wrench is also mapped to thruster cmds)
    double rw_torque_cmd[3]; //!< trick_units(N*m)
    double mtq_dipole_cmd[3]; //!< trick_units(A*m^2)
    /** Commanded RCS wrench in body frame (before per-thruster allocation). */
    double rcs_force_body[3]; //!< trick_units(N)
    double rcs_torque_body[3]; //!< trick_units(N*m)
