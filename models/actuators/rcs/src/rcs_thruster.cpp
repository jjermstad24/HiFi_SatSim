...
{
    double throttle = math::clamp(cmd, 0.0, 1.0);

    if (throttle > 0.01) {
        on_timer += dt;
        off_timer = 0.0;

        if (on_timer > ignition_delay)
            is_on = true;
    } else {
        off_timer += dt;
        on_timer = 0.0;

        if (off_timer > shutdown_delay)
            is_on = false;
    }

    if (!is_on && on_timer > 0.0 && on_timer < min_on_time) {
        is_on = true;
    }
}
...
{
    // ✅ stable thrust model (no feedback amplification loop)
    double throttle = math::clamp(cmd, 0.0, 1.0);
    double thrust = throttle * thrust_vac;
